...
def main(args, is_ray_tune = False, checkpoint_dir=None):
    ...
    for run in range(args.n_runs):
        ...
        plot_directory = f"./{args.description}/plot/fairness_gap"
        os.makedirs(plot_directory, exist_ok=True)
        plot_stats_in_logger(run, logger, index, plot_directory)
        # == SNR ==
        snr_directory = f"./{args.description}/log/snrs"
        snr_name = f"/{args.alg}_{args.dataset}_N{args.num_users}_S{args.shard_per_user}_R{run}.np"
